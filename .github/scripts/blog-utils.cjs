  ...
  try {
    ...
    try {
      ...
      if (match && parseInt(match[1], 10) === parseInt(issueNumber, 10)) {
        ...
        console.log(
          `Found existing file in ${targetRepo.owner}/${targetRepo.repo}: ${filePath}`
        )
      }
    } catch (error) {
      if (error.status !== 404) {
        console.error(
          `Error checking file in ${targetRepo.owner}/${targetRepo.repo}:`,
          error.message
        )
        return false
      }
    ...
    console.log(
      `Successfully synced to ${targetRepo.owner}/${targetRepo.repo}: ${filePath}`
    )
    return true
  } catch (error) {
    console.error(
      `Error syncing to ${targetRepo.owner}/${targetRepo.repo}:`,
      error.message
    )
    return false
  }
...
async function deleteFileFromRepo(
  ...
  try {
    ...
    if (match && parseInt(match[1], 10) === parseInt(issueNumber, 10)) {
      ...
      console.log(
        `Successfully deleted from ${targetRepo.owner}/${targetRepo.repo}: ${filePath}`
      )
      return true
    } else {
      console.log(
        `File ${filePath} in ${targetRepo.owner}/${targetRepo.repo} doesn't match issue #${issueNumber}`
      )
      return false
    }
  } catch (error) {
    if (error.status === 404) {
      console.log(
        `File not found in ${targetRepo.owner}/${targetRepo.repo}: ${filePath}`
      )
      return true // 文件不存在也算成功
    } else {
      console.error(
        `Error deleting from ${targetRepo.owner}/${targetRepo.repo}:`,
        error.message
      )
      return false
    }
